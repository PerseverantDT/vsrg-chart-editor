...
namespace ChartEditor {
    export class Note {
    private:
        NoteType m_type = NoteType::DEFAULT();
        float m_beat;
        float m_holdLength = 0.0F;
        uint8_t m_lane;
    public:
        [[nodiscard]] const NoteType &type() const noexcept { return m_type; }
        ...
        void type(const NoteType &type) noexcept { m_type = type; }
        ...
        Note(const NoteType &type, const float beat, const uint8_t lane) : m_type(type), m_beat(beat), m_lane(lane) {}
        Note(const NoteType &type, const float beat, const float holdLength, const uint8_t lane) : m_type(type), m_beat(beat), m_holdLength(holdLength), m_lane(lane) {}
